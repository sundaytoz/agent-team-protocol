...
> - Codex: `@` 는 (1) config 식별자(`plugin@marketplace`)이자 (2) 설치 플러그인/번들 skill 멘션 접두(cited). 번들 skill 은 `plugin:skill` 콜론 namespace 로 노출(verified-empirical 2026-06-10: 런타임 레지스트리 `atp:task`/`atp:init`), 명시 호출 토큰 `/task`(런타임 보고).
...
| Codex CLI (호출 토큰) | `/task [요청]` (런타임 보고); `@` 멘션 시 `atp:task` 참조 | verified-empirical (codex exec 런타임) / `@` UI form 미관측 |
...
- [verified-empirical 2026-06-10 · codex exec] Codex 번들 skill = `atp:task`/`atp:init` (`plugin:skill` 콜론 namespace, 런타임 레지스트리). 호출 `/task`(런타임 보고). `skills:"./skills/"` 선언 후 재설치 시 skill 노출 확인. 잔여 TODO:실측: `@` 멘션 UI 정확 토큰(대화형 REPL 미관측).
...
| 항목 | 값 |
| --- | --- |
| 호출 | `/task [요청]` (런타임 보고); skill id `atp:task` (`plugin:skill` 콜론, verified-empirical 2026-06-10) |
| 지침파일 | `AGENTS.md` |
...
>
> 번들 skill = `atp:task`/`atp:init` (`plugin:skill` 콜론 namespace — 런타임 레지스트리 직접 확인). 호출 `/task [요청]`(런타임 보고). `skills:"./skills/"` 선언 후 재설치 시 skill 노출 확인됨(충분조건 충족).
> 잔여 TODO:실측: `@` 멘션 UI 의 정확 토큰(대화형 REPL 미관측). env var `PLUGIN_ROOT`/`CLAUDE_PLUGIN_ROOT` 의 skill·agent 본문(hook 외) 가용성.
...
| --- | --- | --- | --- |
| `task` | `/atp:task <요청>` | `/task <요청>` (skill id `atp:task`, verified-empirical) | `/atp:task <요청>` `TODO:실측` |
...
- [ ] Claude Code 안내에는 `/atp:task` (콜론), Codex 안내에는 `/task`(skill id `atp:task`) 가 쓰였는가? (`$`-접두·`@`-멘션 가설 표기 잔존 0)
