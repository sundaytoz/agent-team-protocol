...
> **현재 권위 경로**: `.claude/work-session/<sid>/`
>
> 새 세션·문서·구현 모두 이 경로를 기본값으로 사용한다. 아래 박스는 미래 예약만이다.

> **[미래 이전 예약 — 이번 릴리스 미적용]**
> 플랫폼 중립 경로 `.atp/work-session/<sid>/` 로의 전면 이전은 후속 릴리스에서 결정한다.
> 현재 어떤 문서도 `.atp/work-session/` 을 권위 경로로 쓰지 않는다.
...
> **`@` 주의**: `@` 는 어느 플랫폼에서도 명령 *호출* 문법이 아니다.
>
> - Claude Code: `@` 는 마켓플레이스 설치 구분자(`/plugin install atp@agent-team-protocol`) 전용.
> - Codex: `@` 는 config 식별자(`plugin@marketplace`)와 플러그인 멘션 접두.
> - Gemini: `@<agent_name>` 은 subagent 를 명시 위임하는 문법(`@codebase_investigator ...`) — 명령 호출이 아닌 서브에이전트 호출.
>
> `/atp@task` 같은 표기는 설치/config 문법과 명령 호출 문법을 혼동한 것이므로 쓰지 않는다.
...
| Codex CLI (skill 일반) | `$skill-name` 접두 | cited |
| Codex CLI (플러그인 번들 skill namespace) | **미확정** (`$task` vs `$atp-task`) | **TODO:실측** |
...
| Codex CLI | 플러그인 (`.codex-plugin/plugin.json`; skills/·hooks/·assets/ 는 plugin root) | cited |
...
- [TODO:실측] Codex 플러그인 번들 skill 호출 namespace 정확 표기 (`$task` vs `$atp-task`).
...
단일 agent 는 각 phase 진입 시 아래를 자기 점검하고, phase 종료 시 해당 phase 의 report v1 산출물을 `.claude/work-session/<sid>/` 에 기록한 뒤 다음 phase 로만 전진한다.
...
| 항목 | 값 |
| --- | --- |
| 호출 | `$task <요청>`, `$init` (`$` 접두 skill 멘션) |
| 지침파일 | `AGENTS.md` |
| 플러그인 루트 env | `PLUGIN_ROOT` (+ 호환 `CLAUDE_PLUGIN_ROOT`, hook 한정) |
| 쓰기 데이터 env | `PLUGIN_DATA` (+ 호환 `CLAUDE_PLUGIN_DATA`) |
| 배포 단위 | `.codex-plugin/plugin.json` |
| Tier | A (doc-cited, install-smoke 보류) |

> **Codex 번들 skill namespace — TODO:실측 박스**
>
> `$task` vs `$atp-task` 등 플러그인에 번들된 skill 의 정확한 호출 접두는 공식 문서에 미게시(namespace 역할만 명시). 실제 Codex install 스모크 1회 후 확정. 현재 표기(`$task`)는 잠정이다.
>
...
| --- | --- | --- | --- |
| `task` | `/atp:task <요청>` | `$task <요청>` `TODO:실측` | `/atp:task <요청>` `TODO:실측` |
| `init` | `/atp:init` | `$init` `TODO:실측` | `/atp:init` `TODO:실측` |
...
| `AGENTS.md` | `$task [요청]` (TODO:실측 caveat 포함) |
...
| --- | --- | --- | --- |
| 세션 공유 상태 | `.claude/work-session/<sid>/` | `.claude/work-session/<sid>/` | 현행 사실 경로 |
| 프로젝트 지침 | 플랫폼별 파일 | `CLAUDE.md` | Codex: `AGENTS.md`, Gemini: `GEMINI.md` |
| 플러그인 매니페스트 | 플랫폼별 디렉토리 | `.claude-plugin/` | Codex: `.codex-plugin/` |
...
3. `.claude/work-session/` → `.atp/work-session/` 경로 이전: **이월(이번 릴리스 비목표)**.
...
- [ ] Claude Code 안내에는 `/atp:task` (콜론), Codex 안내에는 `$task` (`$` 접두) 가 쓰였는가?
...
- [ ] `.atp/work-session/` 이 예약 라벨 박스 밖에서 권위 경로로 쓰이지 않는가?
