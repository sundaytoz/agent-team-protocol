...
> - Codex: `@` 는 config 식별자(`plugin@marketplace`) 전용. skill 명시 호출 토큰은 **`$`** (skill 멘션 접두 — cited 공식 docs "type `$` to mention a skill" + 사용자 대화형 실측 `$task`, 2026-06-10). 번들 skill 은 `plugin:skill` 콜론 namespace 로 레지스트리 노출(verified-empirical 2026-06-10: `atp:task`/`atp:init`).
...
| Codex CLI (호출 토큰) | `$task [요청]` (`$` = skill 멘션 접두; `/skills` 로 목록) | verified-empirical (2026-06-10 사용자 대화형 실측) + cited (공식 skills docs) / `$atp:task` 콜론 형태 수용 여부 TODO:실측 |
...
- [verified-empirical 2026-06-10] Codex 번들 skill = `atp:task`/`atp:init` (`plugin:skill` 콜론 namespace, codex exec 런타임 레지스트리). 호출 토큰 = **`$task`** (사용자 대화형 실측 + 공식 docs `$` skill 멘션 접두). `skills:"./skills/"` 선언 후 재설치 시 skill 노출 확인. 잔여 TODO:실측: `$atp:task` 콜론 형태 수용 여부.
...
| 항목 | 값 |
| --- | --- |
| 호출 | `$task [요청]` (`$` skill 멘션 접두 — verified-empirical 2026-06-10 사용자 실측 + cited); skill id `atp:task` (`plugin:skill` 콜론, verified-empirical) |
| 지침파일 | `AGENTS.md` |
...
>
> 번들 skill = `atp:task`/`atp:init` (`plugin:skill` 콜론 namespace — 런타임 레지스트리 직접 확인). 호출 **`$task [요청]`** (`$` skill 멘션 접두 — 사용자 대화형 실측 2026-06-10 + 공식 docs cited). `skills:"./skills/"` 선언 후 재설치 시 skill 노출 확인됨(충분조건 충족).
> 잔여 TODO:실측: `$atp:task` 콜론 형태 수용 여부. env var `PLUGIN_ROOT`/`CLAUDE_PLUGIN_ROOT` 의 skill·agent 본문(hook 외) 가용성.
...
| --- | --- | --- | --- |
| `task` | `/atp:task <요청>` | `$task <요청>` (skill id `atp:task`, verified-empirical 2026-06-10) | `/atp:task <요청>` `TODO:실측` |
| `init` | `/atp:init` | `$init` (skill id `atp:init`) | `/atp:init` `TODO:실측` |
...
| `AGENTS.md` | `$task [요청]` (skill id `atp:task`, verified-empirical) |
...
- [ ] Claude Code 안내에는 `/atp:task` (콜론), Codex 안내에는 `$task`(skill id `atp:task`) 가 쓰였는가? (`/task` 단독·`@`-멘션 가설 표기 잔존 0)
