...
- [platform-adapters.md](../../plugins/atp/docs/development/platform-adapters.md) — capability tier 권위 SSoT (Layer 0~2 전체)

---

## 2026-06-30 실측 노트 — Antigravity IDE 검증 (append-only)

ADR-0006 당시 "Gemini CLI" 로 계획한 Google 호스트의 첫 실측 대상은 **Antigravity IDE** (© 2026 Google, Version 2.2.1)임이 확인됐다.

| 항목 | ADR-0006 계획/추정 | 실측 결과 |
|---|---|---|
| 제품명 | Gemini CLI | Antigravity IDE |
| 호출 문법 | `/ns:cmd` (콜론 추정) | `/atp-task` (하이픈 확인) |
| 지침파일 | `gemini-extension.json` + `commands/*.toml` (F-3PLAT-4 이월) | `GEMINI.md` 직접 읽기 (manifest 불필요) |
| Tier | A-flat (doc-cited, smoke 보류) | A-flat 확정 (§2.5 fallback 동작 확인) |

결정 3(Tier A-flat)은 실측으로 유지. F-3PLAT-4(gemini-extension.json)는 Antigravity IDE 한정 resolved-not-needed.

상세: [ADR-0015](./ADR-0015-antigravity-host-verification.md).
- [agent-team-protocol.md](../../plugins/atp/docs/development/agent-team-protocol.md) — §2.7(research 반전 트리거), §2.8(플랫폼 tier 연결 절)
