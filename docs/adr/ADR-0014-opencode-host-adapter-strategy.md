...
## 구현 완료 — D7 후속 (2026-06-24)

D7 에서 보류한 실제 generator/installer 빌드 및 정식 설치 스모크가 완료됐다.

### 구현 요약

**배포**: npm `bin` CLI(`adapters/opencode/`). `install`/`uninstall` 서브커맨드 제공. `--global`(~/.config/opencode/) / `--project`(.opencode/) 스코프 선택. `--with-graphify`(graphify 에이전트 3개 포함) / `--provider`(tier→slug bake) 옵션.

**변환**: generator 가 canonical 무수정 상태로 유일 emit. tools→opencode permission(per-tool 매핑), `mode: subagent` 전수, description 보존.

**모델**: 기본 `model:` 생략(상속) — ProviderModelNotFoundError 0 실증. `--provider amazon-bedrock` 지정 시만 tier→slug bake.

**토폴로지**: **Tier A-flat** 확정. primary(atp-task command)가 advisor·worker 직접 fan-out. subagent `task:deny`로 재귀 차단 — 실측 확인.

**경로**: plugin-root 변수 부재 → generator가 `@<설치경로>/atp/docs/...` 절대경로로 치환(실측 read OK). `${CLAUDE_PROJECT_DIR}` → 상대경로.

**진입**: `opencode run --command atp-task "<args>"` (primary 경유. `--agent <subagent>` 직접 primary 호출 불가).

### 스모크 결과 (results.md 인용)

정식 설치 스모크 전건 PASS — L1 15/15, L2 7/7, FAIL/skip 0 (검증 결과: `.atp/work-session/20260624-172721/verification/results.md`).

| 레벨 | 범위 | 결과 |
|---|---|---|
| L1 — 골든 단위 + 정적 | node --test(24), AC-01~AC-15 | **15/15 PASS** |
| L2 — opencode 런타임 | AC-08,10,11,09b,16,17,18 | **7/7 PASS** |

주요 실측 확인 항목: command 로드·ProviderModelNotFoundError 0 · agent list 10개 파싱 · fan-out + 재귀차단 · worker bash 거부 · @ref 경로 read · global/project 양 scope · uninstall 잔여 0 · 멱등 재install(md5 동일).

### D7 SSoT 정정

D7 원문 "platform-adapters.md capability matrix 에 opencode 행 추가"는 ADR-0009(2026-06-11 중립화)로 그 matrix가 platform-adapters.md에서 이미 제거됐음을 반영하지 못한 inertia였다. **platform-adapters.md 에 opencode host 행 추가는 비적용 — ADR-0009 결정2("새 호스트 실측 → 신규 ADR 발행")에 따라 본 ADR-0014 accepted 발행 + platform-adapters.md §8 동결이력 포인터 1줄로 충족.** host 행은 영구 추가하지 않는다(중립화 유지).
