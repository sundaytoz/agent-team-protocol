...
일반화 게이트는 변경분(diff) 추가 라인 + 신규 파일명 slug 에 적용하며, 식별자 residual 0 을 commit 전 확인했다. 이 "commit 전 확인" 의 실행 경로(self-grep 게이트)는 [release-checklist §7](../development/release-checklist.md) + [ADR-0012](./ADR-0012-backport-self-leak-grep-gate.md) 가 정의한다 — 본 선언이 dead gate 가 되지 않도록 집행을 회수한 결정.
...
- [agent-team-protocol.md](../../plugins/atp/docs/development/agent-team-protocol.md) — §2.5, §4.4
- [documentation-guidelines.md](../../plugins/atp/docs/development/documentation-guidelines.md) — analysis perspective
