...
※ 번들 skill 은 `plugin:skill` 콜론 namespace(`atp:task`/`atp:init`)로 노출되며 `/task` 로 호출한다 (verified-empirical 2026-06-10, codex-cli 0.138.0).
권위 레퍼런스: atp 플러그인 번들 `docs/development/agent-team-protocol.md`.
<!-- atp:end -->
EOF
      ;;
...
- 생성된 지침파일 목록 + 각 플랫폼 호출 문법 안내:
  ...
  - AGENTS.md 생성/갱신 → "Codex: `/task [요청]` (skill id `atp:task`, verified-empirical 2026-06-10)"
