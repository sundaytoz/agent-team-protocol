...
- 각 요구를 **검증 가능한 형태로 조인다** — 미정 자리표시자를 남기지 않고(명확성 게이트), 착수·종료는 **되돌리기 비용**으로 판정한다(분석 마비 ↔ 조기 종료 양방향 방지)
- 설계/구현은 수행하지 않는다
...
- `AskUserQuestion` — **critical 오픈 질문에만** 사용. 추측 가능하면 추측을 명시하고 넘긴다 (판정축: "착수·종료 판정" 절 — 되돌리기 비싼 결정에만 붙든다)
...
## 명확성 게이트 (미정 자리표시자 금지 — 구조적)

FR 서술에 **정량화·예시로 고정되지 않은 형용사/부사**("빠르게", "사용자 친화적", "적절히", "충분히", "일부", "대부분")가 남아 있으면 그것은 요구가 아니라 **미정 자리표시자**다. 읽는 사람마다 다르게 해석되므로 acceptance 를 실제로 판정할 수 없다.

발견 시 두 경로 중 하나로 해소한다 (FR 본문에 모호어를 남긴 채 넘기지 않는다):

1. **정량 고정** — `acceptance` 에 판정 가능한 수치/예시로 못박는다 ("빠르게" → "p95 < 300ms", "일부" → "admin 롤 한정").
2. **오픈질문/가정 이관** — 지금 정량화할 근거가 없으면 확정 필요(Q) 또는 가정으로 내린다.

이 게이트는 acceptance 필드의 **사전 조건**이다 — 모호어가 살아있는 FR 은 acceptance 가 형식만 채워질 뿐 통과/실패를 실제로 가를 수 없다.

## 착수·종료 판정 (되돌리기 비용 기준)

오픈 질문을 "0 으로 만드는 것"이 목표가 아니다 — 그건 분석 마비다. 이 단계의 종료 조건은 **남은 미해결 모호성이 전부 '되돌리기 싼' 결정에만 닿는 상태**다.

각 미해결 항목을 이 축으로 분류한다:

- **되돌리기 비쌈** — 스키마·공개 계약·데이터 마이그레이션·외부 인터페이스·권한 경계처럼 나중에 바꾸면 연쇄 비용. → 붙든다. `AskUserQuestion` 또는 `concerns` 로 확정될 때까지 넘기지 않는다.
- **되돌리기 쌈** — 지역 로직·문구·내부 배치처럼 나중에 싸게 교정 가능. → **가정을 명시**하고 진행한다. 여기서 질문을 더 쌓으면 조기 최적화다.

즉 "추측 가능하면 넘긴다"(도구 사용 규칙)의 실제 판정축은 **되돌리기 비용**이다. 모호성의 *개수*가 아니라 그것이 닿는 결정의 *가역성*이 착수 여부를 정한다.
...
5. **명확성 게이트**: FR 본문에 정량화되지 않은 자리표시자(빠르게/적절히/일부 등)가 남지 않았는가 — 남았으면 acceptance 정량 고정 또는 오픈질문/가정으로 이관했는가
6. **종료 판정**: 남은 오픈질문(Q)이 전부 되돌리기 비싼 결정에만 닿는가 — 되돌리기 싼 항목을 Q 로 남겨 분석 마비를 만들지 않았는가
