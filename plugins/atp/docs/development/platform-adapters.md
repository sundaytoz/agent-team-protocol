...
- [verified-empirical 2026-06-10 · codex-cli 0.138.0] Codex subagent spawn 실증 — anipang3 세션 20260610-154723 사용자 대화형 전사에서 Codex 가 subagent **2개 spawn 확인**. 축1 Codex 셀 cited → verified-empirical 격상 근거. (spawn 실증 ≠ install 스모크 전부 완료 — 아래 needs_user_verification "세 플랫폼 install 후 task/init 스모크" 항목과 구분 유지.)
...
- **Tier A-flat** (Gemini 전용 변형) = spawn 은 되나 subagent→subagent 재귀 금지. orchestrator 가 advisor 와 worker 를 **모두 직접** 호출하는 평탄 구조(1단 fan-out). 게이트·report 스키마(§8)·검증규율 전량 유지.
...
| Model tier 매핑 | small=`haiku` / medium=`sonnet` / large=`opus` (§1.6, as-of 2026-06); effort 미지원(no-op) |
| Tier | A (확정) |
...
| Model tier 매핑 | 경량/표준/최상위 등급 (§1.6 — 구체 슬러그 미기재); effort=`model_reasoning_effort` (cited) |
...
| Model tier 매핑 | 경량/표준/최상위 등급 (§1.6 — 구체 슬러그 미기재); effort TODO:실측 |
