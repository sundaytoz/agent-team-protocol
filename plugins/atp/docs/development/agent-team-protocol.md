...
- **cwd 오염 방지** (§7): work-session 생성·artifact·verification 등 구조적 산출물 경로는 절대경로 사용. compound `cd X && …` 이후 Bash 호출은 cwd 를 재확립(전체 규율 → §7 cwd 규율 절).
...
| 회고→docs/MEMORY (docs-first) / phase 아티팩트 / 탐색도구 | `## 12.` | 회고·아티팩트·도구 (13/14 인접) | L868 |
...
- 역할:
  ...
  6. 사용자에게 최종 보고
- **예외적으로 직접 수행이 허용되는 것**: 공유 상태 파일 갱신, 사용자 질의, 타이트한 오타/URL/한 줄 수정 같은 마이크로 편집, 메타 작업(회고 결과를 docs/MEMORY 에 반영 — docs-first, §12).
...
| (b) 재발 가능 패턴 | docs 에 교훈 기재(docs-first); 사용자 설정 허용 시 보조로 MEMORY `type: feedback` 추가 | `report.md` `retrospective.memory_candidates[]` |
| (c) 프로토콜 허점 | `report.md` `protocol_feedback[]` 에 구조적 허점 기록. 세션 내 합의 시 개선 설계 진입. 구조적 항목의 cross-session 승격(번들 소스 레포)은 §12 | `user_signals.negative[]` — `structural: true` + `protocol_feedback[]` |
...
| write-capable implementation | ownership 회수 후 scope 축소 self/direct 수행 또는 독립 dispatch; 격리 불가 시 blocked | 동일 write scope의 old/new owner 동시 작업 |
| mandatory verification | Tier B로 동일 검증을 직접 실행; 실행 환경 또는 필수 게이트를 충족할 수 없으면 blocked나 기존 정책이 허용한 `needs_user_verification` | advisor 장애를 이유로 verification skip 또는 L2 실패 종료 |
...
2. 사용자 옵션에 다음을 추가한다:
   - **partial 활용 + 좁은 clean retry** (Recommended): ownership 회수·termination/격리 확인 후 잔여 범위만 명시한 새 invocation + tier 한 단계 다운그레이드(`large` → `medium`, `medium` → `small`). 프롬프트 첫 줄에 "이미 완료(재작성 금지): …" 를 명시해 중복 작성을 막는다.
3. 다운그레이드는 좁은 잔여 범위에서만 적용한다. 충돌 중재·스키마 결정 같은 "잔여인데 큰 결정" 은 동일 tier 를 유지한다.

§2.1 분할 호출 중 소켓 오류는 모델 다운그레이드가 아니라 분할 재시도로 처리한다(§5.7). 본 절의 다운그레이드는 **부분 산출물이 남은 중단**에 한정되며, 사용자 인지 하의 명시 다운그레이드이므로 §5.7 의 silent downgrade 금지와 충돌하지 않는다.
...
목록·표·카탈로그 등 **다항목 산출물에서 한 항목이 결함으로 지적·발견**되면, 그 항목만 제거/수정하고 끝내지 않는다. **동일 출처·동일 생성방식으로 만들어진 나머지 항목이 같은 결함 클래스를 공유하는지 발원 단계에서 전수 재검**한다. 한 항목의 결함은 거의 항상 생성 절차의 결함이며 같은 절차로 만든 다른 항목에 재현된다. (§4.3 집합 전수 체크 AC 가 design 시점의 예방, §4.8 축-완결성 패스가 research 시점의 축 부재 예방이라면, 본 절은 결함 표면화 이후의 사후 전수 재검이다. 단 축이 통째 부재해 항목이 0이면 본 절은 발동되지 않으므로 §4.8 이 그 사각을 사전에서 메운다.)

전수 재검을 수행하는 단계는 **동명이인(homonym) 엔티티를 식별자로 구분**한 채 재검한다 — 같은 이름의 다른 엔티티를 한 항목으로 병합하거나 혼동해 재검 과정에서 새 결함을 주입하지 않는다(재검 worker 의 결함 주입 방지).

전수 재검의 축은 **산출물 축**(같은 표·목록·카탈로그)에 국한되지 않는다. 결함이 **dispatch 로 주입된 사실**(§2.9)에서 비롯한 경우, 재검은 **동일 주입 채널(같은 dispatch 프롬프트)에 함께 주입된 다른 사실 전부**를 포함한다 — 하나의 주입 오류는 같은 프롬프트에 들어간 형제 주입에 재현되기 때문이다. 산출물 측 전수 재검만 수행하고 주입 측 형제 사실을 재검하지 않으면, 같은 발원에서 만들어진 동류 결함이 다른 경로로 잔존한다.

#### 불확실성 보존 (계층 간 격상 금지)

orchestrator 가 하위 산출(조사 결과 등)을 상위 산출(report·design)로 격상할 때 원본의 hedge·신뢰도 마커를 **보존**한다. "추정/미확인" 을 "확정/실제" 로 바꾸는 격상(uncertainty laundering)은 금지한다 — 하류 단계가 미검증 정보를 권위 있는 전제로 오인해 결함이 증폭된다. (본 절은 불확실성의 **출력 방향**(하위→상위 격상)을 다룬다. 대칭짝인 **입력 방향**(orchestrator → advisor dispatch 주입) 오염 규율은 §2.9.)
...
### 2.8 호스트 capability tier 와 위임 토폴로지

§2 의 호출 모델은 호스트 CLI 의 subagent capability 에 따라 평탄화·격하될 수 있다. capability tier 정의, **호스트 자가판정 절차**(spawn 가능? → 재귀 가능? → Tier 결정), 격하 트리거의 **권위 정의는 `platform-adapters.md`** 다. 호스트 고유 문법·env·배포 단위는 열거하지 않는다 — 에이전트가 자기 호스트 규약을 자율 적용한다(과거 3사 실측 데이터는 소스 레포 ADR-0009 부록에 동결 보존).
...
### 2.9 dispatch 사실 주입 규율 (입력 방향 — §2.6 의 대칭짝)

§2.6 의 "불확실성 보존" 이 불확실성의 **출력 방향**(advisor → report, 하위→상위 격상)을 다룬다면, 본 절은 **입력 방향**(orchestrator → advisor 의 dispatch 프롬프트 주입)을 다룬다. dispatch 프롬프트는 "내부 스캐폴딩" 이 아니라 advisor 가 권위 전제로 채택하는 입력이므로, 산출물과 **동일한 출처·hedge 규율**을 적용받는다.

#### 원칙

- dispatch 프롬프트의 고유명사·버전·수치·정책값은 **권위입력(charter / PROMPT / 확인된 문서)에 출처가 있거나 `⚠추정` 마커가 붙은 경우에만** 단정 기재한다. 둘 다 아니면 사실로 넣지 않고 **"조사 포인트" 로만** 둔다 (advisor 가 검증한 뒤 결과로 채운다).
- orchestrator 의 parametric 지식·통념은 권위입력이 아니다. dispatch 에 단정 주입 금지.

#### 2섹션 dispatch 템플릿

dispatch 프롬프트는 두 섹션을 분리해 작성한다:

- **확정 사실 (출처 명시)**: 각 항목에 권위입력 출처를 부착한다.
- **조사 포인트**: 출처 없는 / 미확인 항목. advisor 는 이를 검증 대상으로 다루며, 검증 결과 전에는 권위 전제로 승격하지 않는다.

#### advisor 측 방어 (채택 비대칭 완화)

advisor 는 dispatch 의 "확정 사실" 이라도 **명시 출처가 없으면** 권위 전제로 채택하지 않고 조사 대상으로 강등한다. orchestrator 입력을 무조건 권위로 신뢰하지 않는 것이 하향 주입 오염의 단일 실패점(채택 비대칭)을 완화한다.

advisor 가 입력에 주입된 사실과 **모순되는 사실을 1차 출처로 독립 발견**하면, orchestrator 는 그 모순을 하류 디스패치 전에 reconcile 한다 — 원래 주입 프레이밍이 하류 단계로 재상속되지 않도록 발원 회귀(§2.6)하거나, seed 가정 반전이면 plan 게이트(§2.7 항목 5)를 발동한다. 이미 표면화된 모순을 통합하지 않고 하류로 넘기면, 드러난 신호를 버리는 통합 실패가 된다.

#### 귀속(provenance) 날조 금지

어느 단계의 advisor 든 "명명 출처 X(charter / 약관 / PROMPT 등)가 Y 를 표기 / 명시 / 규정" 형식의 **귀속 주장**을, 명명 출처 X 에 대한 대조 검증 없이 산출물에 기재하지 않는다(검증의 정식화는 §4.3 의 attribution AC). 입력측 주입(출처 없는 단정)과 귀속 날조(없던 출처를 사후 부착)는 **별개 결함 클래스**이므로 각각 차단한다 — 입력이 옳아도 귀속 날조는 독립적으로 발생할 수 있고, 그 역도 같다.

#### 배경

운영 세션에서 orchestrator 가 research dispatch 프롬프트에 권위입력에 없는 운영사명을 단정 주입 → advisor 가 권위 전제로 채택 → 하류 documentation 산출물 4곳에 거짓 귀속이 전파된 사례가 본 절 신설의 직접 계기다. §2.6 이 출력 방향만 커버해 입력 방향 주입 오염이 맹점이었다. 본 사례는 단일 관측(n=1)이며, 구조 근거는 빈도가 아니라 §2.6 의 단방향 가정(아키텍처)에서 도출된다 — `structural-latent`(구조가 허용)이지 `structural-frequent`(반복 관측)는 아니다(§8 `structural_class`, 승격 경로 §12).

### 2.10 doc-저술 시 advisor 트리거 — 분량이 아니라 성격

문서 저술 작업에서 advisor 팀을 붙일지는 **doc 의 분량이 아니라 성격**으로 판단한다. advisor 호출의 이득(전문 렌즈·병렬성)은 (a) 신규 코드/설계/외부 조사가 있거나, (b) 필요한 도메인 지식이 orchestrator 밖에 있을 때 발생한다.

다음 세 조건이 **모두** 성립하면 — (1) 신규 코드/설계 없음(문서 구조가 곧 설계), (2) 외부 조사 불요, (3) 해당 도메인 지식을 orchestrator 가 보유 — 큰 doc(수백 줄)이라도 requirements/research/design/implementation/documentation/verification advisor 를 **전수 skip 하고 orchestrator 직접 저술**하는 것이 우위일 수 있다. 이때 남는 유일한 advisor 가치인 documentation-advisor 의 규율(카테고리 배치·index 링크·frontmatter·내부 링크 실존)은 orchestrator 가 **self-verification 으로 흡수**한다.

의무 2가지:

1. **skip 근거 기록** — 전수 skip 이라도 각 advisor 의 skip 판단을 Advisor Invocation Decision Log 에 1줄씩 남긴다(누락이 아니라 감사 가능한 결정임을 남김).
2. **documentation 규율 self-check** — 저술 후 (카테고리 index 링크 / frontmatter 필수 필드 / 내부 링크 실존 / 상대경로 정합)을 orchestrator 가 직접 점검한다.

이는 §7 의 impl-advisor 직접 실행 임계값(worker 분할을 언제 생략하나)과 같은 형태의 **축소 방향 재량**이다 — 분량이 크다는 이유만으로 advisor 를 붙이지 않는다. 반대로 위 세 조건 중 하나라도 깨지면(신규 설계·조사 필요·orchestrator 밖 지식) doc 이 짧아도 해당 advisor 를 호출한다.
...
- requirements-advisor 또는 design-advisor 가 UI/UX 섹션 작성 시 다음 두 항목을 **분리** 체크:
  ...
  2. **어휘 체크** — 사용자 노출 문자열(description, success/error, embed 필드, 자동완성 라벨) 에서
     - 기술 식별자 노출 여부 (`field_key`, `mode_param`, `format_method`, `model_filename` 등)
     - 도메인 전문어·약어 노출 여부 (내부 기술 약어, 업무 전문어 등)
     - 영어 키워드 잔존 여부 (`timeout`, `format`, `level` 등)
- 판정 기준: "이 챗봇/CLI 를 처음 쓰는 비개발 사용자" 페르소나. 어휘 치환 후보는 `현재 용어 | 사용 위치 | 권장 치환 | 근거` 표로 requirements 에 기록.
...
§4.3 은 *요구에 명시된* 집합의 design 시점 전수성을 예방하고, 요구에 아직 잡히지 않은 카테고리 **축 집합** 자체의 완결성은 research 시점의 §4.8 이, 결함 표면화 후의 사후 항목 재검은 §2.6 이 분담한다(삼각 상호참조).

#### 귀속 주장 검증 AC (attribution check)

design-advisor 는 검증 포인트를 작성할 때, 산출물이 "명명 출처 X 가 Y 를 표기 / 명시 / 규정 / 정의" 형식의 **귀속 주장**을 포함할 가능성이 있으면 다음 AC 1줄을 추가한다:

> "각 귀속 주장을 명명 출처에 grep 대조한다 — 명명 출처에서 0 hit 이면 거짓 귀속(provenance fabrication)으로 판정하고 발원 단계로 회귀(§2.6)한다."

이 AC 는 비용이 낮고 결정적이다. 미확인 입력 사실이 산출물에서 특정 출처에 귀속될 때, 사후 재량 grep 이 아니라 사전 검증 게이트로 포착한다. 입력측 주입 자체의 차단은 §2.9 가 맡고, 본 AC 는 그 입력이 산출물에서 **없던 출처에 귀속되는 별개 결함(provenance fabrication)** 을 잡는다 — 두 게이트는 상보적이다. 단, 출처를 부착하지 않은 단정("운영사는 Z 다")은 grep 대조 대상이 없어 본 AC 로 잡히지 않으므로, 그 클래스는 §2.9 의 입력측 주입 금지로 막는다.
...
### 4.8 카탈로그/열거형 축-완결성 예방 (research 시점)
...
**삼각 상호참조**: §4.3(design 시점·사전·요구에 명시된 집합) / §2.6 다항목 전수 재검(결함 표면화 후·사후·항목) / §4.8(research 시점·사전·요구에 아직 안 잡힌 축) 셋이 완결성 예방을 시점·계기·대상 축에서 분담한다.

**배경**: 타 프로젝트의 다항목 카탈로그 조사에서 한 카테고리 축이 통째로 누락(담당 worker 0)됐고, 사용자 의심이 우발적으로 전수 재검을 유발할 때까지 묻혀 있었다고 보고됐다. 이 누락·수치는 해당 프로젝트 보고자 진술 기반으로 본 레포에서 직접 검증하지 않았으나(§2.6 불확실성 보존), 갭의 구조(축 부재 → 항목 0 → §4.3·§2.6 어느 쪽도 미발동)는 본 절이 메우는 사각과 일치한다.

**단건 사실로의 확장 + 반증 패스 + 수확 점검 (ADR-0018)**: 하류(design/AC/보고서)를 흔드는 것은 카탈로그 축만이 아니라 **권위 전제로 승격되는 단건 사실**(버전·수치·정책값·seed 가정)이다. 본 절의 독립 ≥2 교차확증 원리를 그 단건 사실에도 적용한다 — 독립 출처 미확보 시 `single-source: true` 플래그 기본 부착 + 검증 전 권위 승격 금지. 같은 축의 인접 장치 둘: (a) **반증 패스** — load-bearing 사실의 `확인됨` 격상 직전 반대 증거 탐색 1회(기록 의무, 항목당 1회 상한), (b) **수확 점검** — 취합 시 load-bearing 갭이 남으면 본 절 절차 1 의 "보강 조사" 로 **1회 한정 재분해**(조사 종료를 budget 이 아닌 수확 기준으로 1회 보정 — 단일 패스 fan-out 의 조기종료 사각 완화). 집행처는 셋 다 research-advisor(게이팅 + 자가검증 6·7). **미발동 조건**: 내부 repo 사실(경로:line 직접 검증)·비승격 참고 항목·저위험 조사(§11 게이트 명문화 관례 준수).
...
정책은 플랫폼 중립 **tier** (`small` / `medium` / `large`, S/M/L 약어 병용 가능) 로 판단한다. 구체 모델 슬러그는 호스트가 platform-adapters §6 매핑으로 해석한다 — 본 문서는 특정 벤더의 모델명을 사용하지 않는다.

결정은 결정론적이어야 한다. 단계별 파이프라인:

```
phase 기본 tier (§5.1) → escalation 트리거 (§5.2) → 디스패치 크기 보정 (§5.3)
  → 판단축 4개 오버라이드 (§5.4) → effort 노브 (§5.5) → cap clamp (§5.6)
  → 환경 fallback (§5.7) → 기록 (§5.8)
```

호스트 전용 모델 route 는 이 파이프라인을 대체하지 않는다. 별도 스펙이 있는 host 는 `platform-adapters.md` §7 의 조건부 appendix 를 **자기 host 일 때만** 읽고, route 가 불가능하면 본 §5 의 기존 tier 매핑으로 fallback 한다.

### 5.1 Phase × 기본 tier

| Phase | 기본 tier | 비고 |
|---|---|---|
| analyze (사용자 발화 분류·근본 원인 탐색) | **large** | 판단 비용 큼 |
| design / planning | **large** | trade-off 빈번 |
| code-implementation (스펙 명확) | **medium** | 기계적 작성 |
| validation — 정적 체크·타입·테스트 | **medium** | rote |
| validation — 런타임·웹·시나리오 | **large** | 주관적 회귀 판단 |
| docs-sync | **medium** | 형식 위주 |
| graphify 실행 | **medium** | 도구 실행 |
| graphify 필요성 판단 | **large** | 메타 판단 |

`small` 은 phase 기본값에 없다 — 추출·포맷 변환 같은 좁고 기계적인 판단에서 §5.4 오버라이드로만 진입한다. 100% 결정론이면 subagent 자체가 불필요하다(§5.3 첫 행 — 직접 실행).

### 5.2 Escalation 트리거 (1단 상승)

phase 기본 tier 위에 다음 중 **하나라도** 해당되면 1단 상승: **small → medium → large** (large 가 천장, cap §5.6 한도 내). 상승 사유를 보고서 `model_choice.escalation_reason` 에 기록.
...
phase·escalation 결과 위에 크기로 보정. 디스패치 크기는 tier 와 직교한 "묶음 크기" 축이므로 라벨에 `-batch` 접미를 붙여 tier 명명(small/medium/large)과 구분한다. 기존 파일 수 단일축 루브릭은 폐기하고, 파일 수는 보조 신호로만 본다.

| 디스패치 크기 | 조건 | 처리 경로 | tier 보정 |
|---|---|---|---|
| `direct` | tool calls < 5 + 단일 파일 + 스펙 명확 | orchestrator 직접 (§1 마이크로 편집 예외) | n/a — orchestrator 본 모델로 처리 |
| `s-batch` | 5–20 tool calls + 스펙 명확 | worker 디스패치 | `medium` 유지 |
| `m-batch` / `l-batch` | > 20 tool calls 또는 판단 사슬 다단 | worker 디스패치 | `large` 로 상승 |
| `parallel` | 독립 영역 2–3개 병렬 | 영역별 worker 디스패치 | escalation 미발동 시 모두 `medium` |

호출 크기 제한은 tier 와 독립이다 — 묶음이 커서 tier 를 올렸더라도 §2.1 분할 임계(예상 >1,000 라인 / >8분)에 걸리면 의미 단위로 분할한다. 상위 tier 라고 디스패치를 길게 줘도 되는 것이 아니다.

### 5.4 판단 천장 결정 — 4개 판단축 (오버라이드)

Phase 이름만으로 tier 를 정하지 않는다. tier 는 "이 호출 안에 남은 미결 판단의 수" = **판단 천장**이다. 본 항이 §5.1 과 충돌하면 **본 항 우선**. orchestrator 가 디스패치 전에 판단을 끝낼수록 worker tier 를 낮출 수 있다.

1. **판단 밀도** — 타깃·변환 규칙·기대 출력이 사전 확정됐는가.
   확정 → 하위 tier. "다음에 뭘 볼지" 스스로 결정(탐색·root-cause) → `large`.
2. **결정론적 검증 가능성** — 출력이 기계 검증 가능(grep/lint/스키마)인가.
   가능 → tier 낮추고 게이트에 투자. 재검증 불가(설계 trade-off·의미 감사) → tier 상승.
3. **오류의 침묵성** — 틀리면 시끄럽게 실패(parse error)하는가, 조용히 실패(lint 통과·런타임만 발화)하는가.
   시끄러움 → 하위 tier 충분. 조용함 → `large` 의미 감사.
   자가 점검: "이 worker 가 틀리면 무엇이 그걸 잡는가?" — 답이 없으면 tier 상승 또는 검증 단계 추가.
4. **컨텍스트 폭** — 단일 파일 국소 변환 → 하위 tier + 좁은 프롬프트.
   다파일 교차 추론·취합·충돌 중재 → 상위 tier (분할하면 교차 정보가 단절돼 품질 하락).

휴리스틱: 결정론 100% 면 subagent 불필요(직접 실행). 디스패치 전에 결정을 앞당기면 tier 한 단계 절감. 불확실 + 출력 게이트 있으면 `medium` 시작 → §5.2 트리거 적중 시 상승. 게이트가 없으면 처음부터 상승.

### 5.5 effort — 직교 노브

tier 가 "판단 천장" 이라면 effort 는 "한 판단을 얼마나 깊게 파는가" 다. 두 축은 직교한다. effort 값: `low` / `medium` / `high`.

- **반박·검증·root-cause** ("이 claim 을 무너뜨려봐") → high effort.
- **추출·포맷 변환** → low effort (높여봐야 토큰 낭비).
- **중간 tier + high effort** 가 유효한 구간 = 좁고 깊은 단일 판단(예: "이 한 건이 재현되는가").
- 판단 천장이 필요한 곳(설계 옵션 공간 탐색)은 effort 로 보충 불가 — tier 를 올린다.
- 최고 effort(플랫폼이 xhigh 류를 제공하면)는 오판 비용이 큰 경우에만: 대규모 마이그레이션·출처 충돌 감사·비가역 배포 판단.

**no-op 규칙**: effort 노브를 노출하지 않는 플랫폼/모델에서는 effort 값을 기록만 하고 런타임에서 무시한다(no-op). effort 미지원이 호출 실패를 유발하지 않는다 — 동일 model_choice 블록을 모든 플랫폼이 공통 작성한다. 지원 여부는 호스트가 자가판정한다(platform-adapters §6).

### 5.6 cap — orchestrator tier 상한 집행

orchestrator 는 **자신에게 할당된 모델의 tier 를 초과하는 tier 를 subagent 에 지정할 수 없다.** escalation(§5.2)·디스패치 보정(§5.3)·판단축(§5.4) 결과가 cap 을 초과하면 cap 으로 **자동 clamp** 한다.

1. **자기 tier 판정**: orchestrator 는 자기 실행 모델 슬러그를 platform-adapters §6 매핑으로 tier 로 해석한다(예: 자기 모델이 표준 등급이면 자기 tier=medium).
2. **clamp**: 산출 tier > 자기 tier 이면 자기 tier 로 내린다. model_choice 에 `capped: true` + `capped_from: <원래 산출 tier>` 를 기록한다.
3. **세션 최종 보고 노출**: capped 가 발생한 invocation 은 orchestrator 최종 보고에 "tier 상한 적용: inv-NNN <요청 tier> → <cap tier> (사유: orchestrator 모델 한계)" 로 노출한다.
4. **판정 불가 안전 폴백**: 자기 모델 슬러그를 알 수 없거나 §6 매핑 해석이 불가한 호스트에서는 tier override 를 지정하지 않고 **parent 모델 상속**만 사용한다. 상속은 정의상 cap 을 초과할 수 없으므로(자식이 부모 모델을 그대로 씀) 안전하다. 이 경우 `resolved_model: inherit`.

**§5.7 silent downgrade 금지와의 정합**: cap clamp 는 비용 사유 임의 다운그레이드가 아니다. 사용자가 *세션 모델 선택으로 이미 표명한 상한*의 집행이다 — 사용자 의사에 반하지 않으므로 silent downgrade 금지(§5.7) 위반이 아니다. clamp 는 보고에 노출되므로 silent 도 아니다.

### 5.7 환경 Fallback

- **확정되지 않은 모델 슬러그를 그대로 넘기지 않는다.** tier→슬러그 해석(platform-adapters §6)이 불가하거나 호출 환경의 whitelist 에 슬러그가 없으면 **parent 모델 상속** 으로 fallback 하고 `fallback_reason` 에 사유 기록.
- **호스트 전용 optional route 실패는 기존 tier 매핑으로 fallback 한다.** host 전용 appendix 가 특정 모델 route 를 제안하더라도 availability, permission, quota, plan, research-preview 제한 또는 호출 실패가 있으면 §6 tier 매핑으로 돌아가고 `fallback_reason` 에 사유를 기록한다.
- **비용 사유로 `large` → `medium` silent 다운그레이드 금지.** large tier 가 환경 미지원이면 **사용자 확인 후** 명시 다운그레이드. (cap clamp 는 다운그레이드가 아니라 상한 집행 — §5.6.)
...
model_choice:
  ...
  dispatch_size: <direct|s-batch|m-batch|l-batch|parallel>   # tier 와 직교한 묶음 크기 (§5.3)
  tier: <small|medium|large>            # 판단 천장 (§5.1~5.4)
  effort: <low|medium|high|null>        # 직교 노브 (§5.5). 미지원 플랫폼은 기록만·런타임 무시
  resolved_model: <호스트가 해석한 실제 모델 슬러그|inherit>   # platform-adapters §6 매핑 결과
  capped: <true|false>                  # §5.6 cap clamp 발생 여부
  capped_from: <원래 산출 tier|null>    # capped=true 일 때 clamp 이전 tier
  escalation_reason: <한 줄 | null>     # §5.2 상승 트리거 적중 시
  fallback_reason: <한 줄 | null>       # §5.7 환경 fallback 시
  rationale: <한 줄>
```

`tier` 와 `resolved_model` 분리 이유: tier 는 플랫폼 중립 의사결정값, resolved_model 은 그 호출이 실제 무슨 슬러그로 떨어졌는가의 사후 기록 — 플랫폼 간 비교·디버깅·retro 보정 루프(rationale + 재작업 여부를 retro 가 읽고 phase 기본값을 프로젝트별로 갱신)에 둘 다 필요하다.

**기본 지침**: 불확실하면 `medium` 에서 시작, §5.2 트리거 적중 즉시 `large` 상승(cap §5.6 한도 내). 비용 하한선 유지하되 §5.7 silent downgrade 금지가 우선.
...
**외부 게시 전 dry-run 검증 규율**: `npm publish` / `docker push` 등 패키지·이미지 공개 게시는 직전에 반드시 dry-run(`npm pack --dry-run` 등)으로 tarball·번들 내용물을 검증한다. 런타임 의존 파일(canonical 소스·설정 등) 누락이 0 임을 확인한 후 게이트를 통과시킨다. 세부 검증 전략은 `verification-strategies.md` 참조.
...
`sid` = ISO 타임스탬프 `YYYYMMDD-HHMMSS` (프로젝트 타임존 기준). 세션 시작 시 orchestrator 가 디렉토리 생성. 타임스탬프 뒤 **kebab-case 슬러그 접미사는 허용**한다(예: `20260721-171300-dual-axis-review`) — 정렬·중복 방지 키는 타임스탬프 부분이고 접미사는 사람용 라벨이다(운영 관행 명문화).
...
- work-session 생성(`mkdir -p`), artifact Write, verification 등 **구조적 산출물 경로는 절대경로 사용**. 상대경로는 cwd 가 변한 뒤에도 그 자리에 그대로 쓰인다.
- compound `cd X && <명령>` 이후 Bash 호출은 cwd 전제를 재확립(`cd <REPO_ROOT>` 또는 `pwd` 검증)한다.
- cwd 의존 탐색(`find`/`ls`/`glob`) 결과가 예상과 다르면 **cwd 오염을 1순위로 의심**하고 `pwd` 로 확인한다.
...
# **spawn 된 모든 subagent 는 예외 없이 invocation 으로 기록한다(플랫폼 불문 의무)** —
#   layer(advisor|worker) + parent_invocation_id 를 채운다. advisor 만 기록하고 그 하위
#   spawn 을 누락하면 §8 위반이다. 모든 호스트 CLI 에 동일 적용.
- id: inv-001
  ...
  concerns: []
  model_choice:                # §5.8 스키마
    phase: <...>
    dispatch_size: <direct|s-batch|m-batch|l-batch|parallel>
    tier: <small|medium|large>
    effort: <low|medium|high|null>
    resolved_model: <실제 모델 슬러그|inherit>
    capped: <true|false>
    capped_from: <원래 산출 tier|null>
...
user_signals:
  ...
  negative:
    - quote_or_paraphrase: <사용자 발화 한 줄>
      ...
      structural_class: structural-latent | structural-frequent | null  # 옵셔널 — structural:true 일 때만. 구조가 허용(latent, 단일 관측 포함) vs 반복 관측(frequent) 구분. n=1 을 빈발로 오독 방지 (§2.9 배경, 승격 §12)

# Retrospective
- signals: { positive: [], negative: [] }   # retro 가 user_signals 를 구조화해 옮김
- what_went_well: []
- what_to_improve: []
- memory_candidates: []       # { name, type, description, body_draft, rationale_for_saving, signal_source, docs_sync_target, memory_optional }
                              # docs-first: docs_sync_target 이 기본 sink(가능한 한 docs 경로). docs 화 부적절 시만 null.
                              # memory_optional: true 면 사용자 memory 설정 허용 시 보조로 memory 기록 권고(강제 아님).
- protocol_feedback: []       # structural 부정 시그널 기반
- applied_changes: []         # docs / MEMORY(설정 허용 시) / SKILL / agent 수정 목록
```

### 경량 프로파일 — 조사·고찰 전용 세션 (`profile: research-lite`)

코드 변경 0 인 **조사·고찰 전용 세션**은 풀 스키마가 과중해 준수 갭(보고서 자체 누락)을 유발한다 — 규범 부적합이 개인 일탈보다 큰 구조 요인이라는 진단(2축 리뷰 세션 20260721-171300)에 따른 옵셔널 프로파일. frontmatter 에 `profile: research-lite` 를 명시하고 다음으로 축약할 수 있다:

- **필수 유지**: 헤더(schema_version·session_id·started_at/ended_at·user_request) / `Summary` / `Invocations`(spawn 전수 기록 의무는 동일 — 단 항목당 `id`·`layer`·`name`·`input_digest`·`output_digest`·`artifacts` 로 축약 가능, **dispatch 프롬프트 요지를 input_digest 에 남긴다** — 독립성 주장의 사후 검증 가능성 확보) / `Decisions`(있으면).
- **생략 가능**: `model_choice` 세부(단 `tier`+`rationale` 1줄 권장), `token_usage`, `Conflicts`/`Regression`(발생 시엔 기록), verification 관련 섹션(코드 변경 0 전제 — 1줄이라도 생기면 풀 스키마로 승격).
- 기존 세션 보고서 소급 수정 금지는 동일. 사후 재구성 기록은 `retroactive: true` 라벨 의무(provenance 오염 방지).
...
**게이트 명문화 관례**: agent 파일이나 프로토콜에 게이트/판정 규칙을 신설·개정할 때는 발동 조건과 함께 **미발동 조건을 같은 절에 명기**한다 (§4.3 "집합이 포함된 요구사항"·§4.8 "열거형/카탈로그만"·§2.10 "세 조건 전수 성립 시만" 의 기존 관행 명문화 — 미발동 조건 없는 게이트는 전 산출에 과적용되거나 반대로 사문화된다).
...
6. **구조적 protocol_feedback 의 cross-session 승격**: `report.md` `protocol_feedback[]` 에 `structural: true` 로 기록된 항목은 세션 report 에만 남으면 다음 세션·다른 프로젝트에 도달하지 못한다(번들은 전역/ephemeral 캐시이며, 프로젝트 지침파일·memory 선반영은 해당 프로젝트에만 일반화된다). 따라서 `structural: true` 항목은 세션 종료 시 **번들 소스 레포의 이슈/PR 로 승격**하는 것을 기본으로 한다.
   - 승격은 outward action 이므로 **사용자 확인 게이트**(§6)를 거친다.
   - 승격 전 임시 완화로 프로젝트 지침파일 / memory 선반영은 허용하되, 이는 일반화가 아니라 **로컬 스톱갭**임을 report 에 명시한다.
   - 승격 항목에는 `structural_class`(§8)로 `structural-latent`(구조가 허용) / `structural-frequent`(반복 관측)를 구분 표기해, 단일 관측(n=1)을 빈발로 오독하지 않게 한다.
...
- [platform-adapters.md](./platform-adapters.md) — 호스트 capability tier 정의·자가판정 절차 + 모델 tier 매핑(§6) (Tier 정의 SSoT — §2.8 에서 참조)
- [codex-lifecycle-routing.md](./codex-lifecycle-routing.md) — Codex host의 §2.5 lifecycle 관측·종결·독립 invocation 매핑 appendix
