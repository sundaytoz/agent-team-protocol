...
|---|---|---|---|
| advisor 호출 분할 / 병렬 / open_questions / system-reminder / 호출실패 회복 / dispatch 사실 주입 | `## 2.` | 호출 모델 전반 + 하위 절(2.1~2.9) | L88 |
...
2. 위 옵션 (a)~(c) 외에 다음을 추가한다:
   - **(a') partial 활용 + 좁은 재호출** (Recommended): 잔여 범위만 명시한 새 프롬프트 + tier 한 단계 다운그레이드(`large` → `medium`, `medium` → `small`). 프롬프트 첫 줄에 "이미 완료(재작성 금지): …" 를 명시해 중복 작성을 막는다.
