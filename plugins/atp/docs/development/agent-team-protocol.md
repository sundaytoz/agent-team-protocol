...
에이전트 frontmatter 의 `model:` 필드는 **비워둔다.** Orchestrator 가 호출 시점에 호스트 CLI 의 per-call override 문법으로 지정한다(플랫폼별 문법·매핑: `platform-adapters.md` §1.6).

정책은 플랫폼 중립 **tier** (`small` / `medium` / `large`, S/M/L 약어 병용 가능) 로 판단한다. 구체 모델 슬러그는 각 플랫폼이 platform-adapters §1.6 매핑으로 해석한다 — 본 문서는 특정 벤더의 모델명을 사용하지 않는다.
...
**no-op 규칙**: effort 노브를 노출하지 않는 플랫폼/모델에서는 effort 값을 기록만 하고 런타임에서 무시한다(no-op). effort 미지원이 호출 실패를 유발하지 않는다 — 동일 model_choice 블록을 모든 플랫폼이 공통 작성한다. 지원 여부는 platform-adapters §1.6 참조.
...
1. **자기 tier 판정**: orchestrator 는 자기 실행 모델 슬러그를 platform-adapters §1.6 매핑으로 tier 로 해석한다(예: 자기 모델이 표준 등급이면 자기 tier=medium).
...
4. **판정 불가 안전 폴백**: 자기 모델 슬러그를 알 수 없거나 §1.6 매핑이 미정인 플랫폼에서는 tier override 를 지정하지 않고 **parent 모델 상속**만 사용한다. 상속은 정의상 cap 을 초과할 수 없으므로(자식이 부모 모델을 그대로 씀) 안전하다. 이 경우 `resolved_model: inherit`.
...
- **확정되지 않은 모델 슬러그를 그대로 넘기지 않는다.** tier→슬러그 해석(§1.6)이 불가하거나 호출 환경의 whitelist 에 슬러그가 없으면 **parent 모델 상속** 으로 fallback 하고 `fallback_reason` 에 사유 기록.
...
model_choice:
  ...
  resolved_model: <플랫폼이 해석한 실제 모델 슬러그|inherit>   # platform-adapters §1.6 매핑 결과
...
#   spawn 을 누락하면 §8 위반이다. Codex/Gemini 등 모든 호스트 CLI 에 동일 적용.
- id: inv-001
