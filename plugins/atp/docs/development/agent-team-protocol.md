...
**배경**: 타 프로젝트의 디스코드 봇 카탈로그 조사에서 "초대 추적(invite tracking)" 축이 통째로 누락(담당 worker 0)됐고, 사용자 의심이 우발적으로 전수 재검을 유발할 때까지 묻혀 있었다고 보고됐다. 이 누락·수치는 해당 프로젝트 보고자 진술 기반으로 본 레포에서 직접 검증하지 않았으나(§2.6 불확실성 보존), 갭의 구조(축 부재 → 항목 0 → §4.3·§2.6 어느 쪽도 미발동)는 본 절이 메우는 사각과 일치한다.
