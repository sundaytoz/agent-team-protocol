...
- **코드·식별자 변경 시 사람용 문서 동반 갱신**: 패키지명·명령·경로 등 식별자를 변경하면 코드·번들 레퍼런스와 함께 **사람용 문서 전수**(README, `docs/usage/**`, 설치 체크리스트·FAQ 등)를 동반 갱신 대상에 포함한다. 다국어 병기 문서(`*.en.md`)는 짝으로 함께 갱신한다. 누락 시 사용자가 구 식별자로 설치를 시도해 혼란이 발생한다.
...
- [ ] (식별자·명령·경로 변경 시) 사람용 문서(README, `docs/usage/**`, FAQ) 와 다국어 짝 문서(`*.en.md`)를 동반 갱신했는가?
